...
/*   Updated: 2018/05/15 18:33:18 by nrouzeva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*find_new_place(t_page *cur_page, t_block *cur, size_t size_m, size_t size_r)
{
	t_block *check_next;
...
void	*search_and_copy(t_page *begin, size_t size_m, void *ptr, size_t size_r)
{
	...
	if ((cur_page = find_page(cur_page, size_m, ptr, &prev)))
		...
		{
			if ((ret = find_new_place(cur_page, cur, size_m, size_r)))
				return (ret);
			ret = ft_malloc(size_r);
			ft_memcpy(ret, (void*)cur + sizeof(t_block), cur->size);
			cur->use = 0;
