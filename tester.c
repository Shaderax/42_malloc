...
/*   Updated: 2018/05/21 18:19:43 by nrouzeva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int argc, char **argv)
{
	...
	while (++index < TINY_MALLOCED - 1)
		tiny[index] = malloc((int)(rand() / (double)RAND_MAX * (MAX_TINY - 1)));
	index = -1;
	while (++index < SMALL_MALLOCED)
		small[index] = malloc((int)(rand() / (double)RAND_MAX * (MAX_SMALL - 1)));
	index = -1;
	while (++index < LARGE_MALLOCED)
		large[index] = malloc((int)(rand() / (double)RAND_MAX * (300000 - 1)));

	index = -1;
	rande = (int)(rand() / (double)RAND_MAX * (TINY_MALLOCED - 1));
	while (++index < rande)
		free(tiny[(int)(rand() / (double)RAND_MAX * (TINY_MALLOCED - 1))]);
	index = -1;
	rande = (int)(rand() / (double)RAND_MAX * (SMALL_MALLOCED - 1));
	while (++index < rande)
		free(small[(int)(rand() / (double)RAND_MAX * (SMALL_MALLOCED - 1))]);
	index = -1;
	rande = (int)(rand() / (double)RAND_MAX * (LARGE_MALLOCED - 1));
	while (++index < rande)
		free(large[(int)(rand() / (double)RAND_MAX * (LARGE_MALLOCED - 1))]);
//	show_alloc();
	
	index = -1;
	rande = (int)(rand() / (double)RAND_MAX * (TINY_MALLOCED - 1));
	while (++index < rande)
		tiny[index] = realloc(tiny[index], (int)(rand() / (double)RAND_MAX * (TINY_MALLOCED - 1)));
	
	index = -1;
	rande = (int)(rand() / (double)RAND_MAX * (SMALL_MALLOCED - 1));
	while (++index < rande)
		small[index] = realloc(small[index], (int)(rand() / (double)RAND_MAX * (SMALL_MALLOCED - 1)));

	index = -1;
	rande = (int)(rand() / (double)RAND_MAX * (LARGE_MALLOCED - 1));
	while (++index < rande)
		large[index] = realloc(large[index], (int)(rand() / (double)RAND_MAX * (LARGE_MALLOCED - 1)));
